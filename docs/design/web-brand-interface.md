...
사용자가 지정한 [공유 대화](https://chatgpt.com/s/cx_6aaff39a83588191b3d5f4cee720fccb)의 최종 제작 커밋 `aeb565a62b9499ba58b58e8ccd235a14e5b2e4a0`에서 `2.0.0-review` PNG 세 개를 바이트 변경 없이 가져왔다. 앞서 만든 임시 인라인 SVG는 제거했다. 전체 캐릭터 PR #32를 병합하지 않고 사용자가 요청한 정지 이미지 자산만 동일 경로로 도입했다.

- `action-holding-source.png`: Figma 정지 원본 출력. 홈·로그인·감지의 브랜드 그림에 사용하며 조치 완료나 실제 아기 상태를 의미하지 않는다.
- `observation-calm.png`: 실제 Lottie 0 프레임. 시각이 확인된 최신 CALM 관찰에만 64px로 표시한다. 오래됨·모순·시각 모름은 이 그림을 표시하지 않는다.
- `ai-hungry.png`: 실제 Lottie 0 프레임. 완료된 분석의 첫 후보가 `hungry`일 때만 원인 후보 설명과 함께 표시한다. 판단 유보·실패·진행 중에는 표시하지 않는다.

원본의 피부·의복 색과 실루엣, 투명 배경을 그대로 사용한다. 화면의 핑크 원형 배경은 CSS로 배치하며 이미지 자체를 재해석하거나 수정하지 않는다. 그림은 장식용 빈 대체 텍스트이며 상태·출처는 인접 HTML 텍스트로 전달한다. 이번 변경은 정지 이미지 적용이다. 두 Lottie의 자동 재생이나 미완료 안아주기 모션, 전체 22종 완료를 뜻하지 않는다.
