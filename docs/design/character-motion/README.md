...
상태: 디자인 연구·이미지 초안·제작 명세 및 **React·SVG 모션 프로토타입**. 실제 API·기록 연동과 A-12 전체 인수는 미완료.
범위: A-12 캐릭터 제작, A-07 표현, A-04 정적 로그의 디자인 접점.

### 실행 가능한 후속 제작물 (2026-09-20)

[프로토타입 실행·검수 기록](runtime-prototype.md)에 부위별 SVG 원본, 22개 정지 자산, 합성 입력 기반 웹 플레이어와 남은 연동 범위를 정리했다. `apps/web`에서 `npm run dev` 후 `/design/character-motion`을 열면 확인할 수 있다. 아래 2~12절의 ‘이번 제작/수정’ 기록은 PNG·명세 작성 당시의 이력이며, 최신 구현·검증 범위는 후속 문서를 따른다.
