...
27회 이후의 비교기·최종 재학습·봉인 테스트 평가·독립 전체 가중치 내보내기는
`post-training` 명령으로 이어진다. 실행·재개·검증 범위는 [후속 실행 안내](POST_TRAINING.md)를
따른다. 실제 완료 여부는 외부 출력 폴더의 `post_training/completion.json`과 그 해시 연결로
판단한다.

2026-09-20 실제 완료·성능·로컬 모델 위치와 남은 서비스 작업은
[실행 결과](RESULTS_20260920.md)에 기록했다. 낮은 희소 라벨 성능을 배포 준비 완료로
해석하지 않는다.
