...
function show(isOwner = true) {
  return render(
    <QueryClientProvider client={queryClient}>
      <BabyProfile baby={baby} isOwner={isOwner} />
    </QueryClientProvider>,
  );
}
...
describe("baby profile editing", () => {
  ...
  it("prevents duplicate submission while preserving the version and idempotency key", async () => {
    ...
    expect(request.params.header["Idempotency-Key"]).toBe(
      request.body.client_request_id,
    );
    await act(async () =>
      finish(ok({ ...baby, alias: "새 이름", version: 2 })),
    );
  ...
  it("requires explicit adoption of an authorized fresh version after conflict", async () => {
    ...
    client.PATCH.mockRejectedValueOnce(conflict).mockResolvedValueOnce(
      ok({ ...baby, version: 3 }),
    );
    client.GET.mockResolvedValue(
      ok({ items: [{ baby: { ...baby, alias: "다른 이름", version: 2 } }] }),
    );
    show();
    fireEvent.submit(await openAndEdit());
    fireEvent.click(
      await screen.findByRole("button", { name: "최신 값으로 다시 작성" }),
    );
    expect(
      (screen.getByRole("textbox", { name: "아기 이름" }) as HTMLInputElement)
        .value,
    ).toBe("다른 이름");
    fireEvent.click(screen.getByRole("button", { name: "변경 저장" }));
    await waitFor(() => expect(client.PATCH).toHaveBeenCalledTimes(2));
    expect(client.PATCH.mock.calls[1]![1].body.version).toBe(2);
    expect(client.PATCH.mock.calls[1]![1].body.client_request_id).not.toBe(
      client.PATCH.mock.calls[0]![1].body.client_request_id,
    );
  });
