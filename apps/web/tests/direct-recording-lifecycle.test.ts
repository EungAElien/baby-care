...
let track: { stop: ReturnType<typeof vi.fn>; onended: (() => void) | null; onmute: (() => void) | null };
...
class Recorder {
  ...
  stop() { this.ondataavailable?.({ data: new Blob(["audio"]) }); this.state = "inactive"; this.onstop?.(); }
}
beforeEach(() => {
...
describe("foreground-only direct recording", () => {
  it("reports permission denial with a recovery action and never reports recording", async () => {
    ...
    const state = vi.fn();
    await expect(new DirectRecording(state).start()).rejects.toThrow("마이크 권한이 거부됐어요");
  ...
  it.each(["hidden", "offline", "ended", "muted"])("discards interrupted audio after %s and releases the microphone", async (reason) => {
    const state = vi.fn();
    const capture = new DirectRecording(state);
    const pending = capture.start();
    const rejected = expect(pending).rejects.toThrow(/다시 녹음/);
    await vi.waitFor(() => expect(state).toHaveBeenCalledWith("recording"));
    if (reason === "hidden") { page.hidden = true; page.dispatchEvent(new Event("visibilitychange")); }
  ...
  it("does not request microphone permission from an already hidden page", async () => {
    page.hidden = true;
    await expect(new DirectRecording().start()).rejects.toThrow("화면을 다시 연");
