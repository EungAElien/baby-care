...
describe("source and missing-data semantics", () => {
  it("does not promote REAL configuration into proof of model execution", () => {
    ...
    view.rerender(<SourceBadge inferenceMode="REAL" inferenceExecuted={false} dataOrigin="DEMO" />);
    expect(screen.getByText("예시 음원 · 모델 미실행")).toBeTruthy();
    view.rerender(<SourceBadge inferenceMode="REAL" inferenceExecuted dataOrigin="USER" />);
  ...
  it("keeps absence, unknown quantity and recorded zero separate", () => {
    ...
    expect(feedingAmount({ ...base, feeding: { ...base.feeding, record_count: 1, total_recorded_ml: null } })).toBe("양 정보 부족");
    expect(feedingAmount({ ...base, feeding: { ...base.feeding, record_count: 1, known_amount_count: 1, total_recorded_ml: 0 } })).toBe("0 mL");
  });
});

function luminance(hex: string) {
  const linear = hex.match(/[a-f\d]{2}/gi)!.map((byte) => { const n = parseInt(byte, 16) / 255; return n <= .04045 ? n / 12.92 : ((n + .055) / 1.055) ** 2.4; });
...
describe("brand contrast and prohibited surfaces", () => {
  ...
  const colors = Object.fromEntries([...css.matchAll(/--palette-([\w-]+):\s*(#[a-f\d]{6});/gi)].map((match) => [match[1], match[2]!]));
  it.each([["black", "white"], ["black", "pink"], ["black", "brick"], ["white", "navy"], ["white", "cobalt"], ["black", "sky"], ["white", "wine"], ["gray-700", "white"], ["gray-700", "gray-50"], ["white", "pressed"]])("%s on %s meets normal-text contrast", (fg, bg) => {
  ...
  it("focus and control outlines meet 3:1, including focus on pink", () => {
    ...
    expect(contrast(colors["gray-500"]!, colors.white!)).toBeGreaterThanOrEqual(3);
  });
