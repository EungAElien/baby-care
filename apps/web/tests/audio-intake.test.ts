...
} from "../src/lib/audio/storage-upload";
...
const grant = (
  method: "STANDARD" | "TUS",
  bytes = 25_000_000,
): components["schemas"]["UploadGrant"] => ({
  upload_id: "upload",
  audio_id: "audio",
  bucket: "baby-audio",
...
const asset = (
  status: components["schemas"]["AudioAsset"]["status"] = "ALLOCATED",
...
describe("server-granted Storage boundary", () => {
  it("rejects wrong origins, credentials, query strings, paths and methods before a token is read", async () => {
    ...
    for (const upload_endpoint of altered) {
      ...
      await expect(
        new StorageUpload(invalid, identity, config, auth).send(
      ...
      validateUploadEndpoint(
        {
          ...grant("TUS"),
          upload_endpoint:
            "https://project.storage.supabase.co/storage/v1/upload/resumable/forged",
        },
        config,
      ),
    ).toThrow();
  ...
  it("uses 6 MiB chunks except the last, current token and no sensitive TUS metadata", async () => {
      ...
      vi.fn(async (_url: string, init: RequestInit) => {
        ...
        if (init.method === "POST")
          return new Response(null, {
            status: 201,
            headers: { Location: "/storage/v1/upload/resumable/session" },
          });
        ...
        return new Response(null, {
          status: 204,
          headers: { "Upload-Offset": String(previous + size) },
        });
      }),
    );
    const blob = new Blob(
      [new Uint8Array(TUS_CHUNK_BYTES), new Uint8Array(5)],
      { type: "audio/webm" },
    );
    const progress: number[] = [];
    await new StorageUpload(grant("TUS"), identity, config, auth).send(
      blob,
    ...
    ).toEqual([TUS_CHUNK_BYTES, 5]);
    expect(
      requests.every(
        (request) =>
          request.headers.get("Authorization") === "Bearer current-token" &&
          request.headers.get("x-upsert") === "false",
      ),
    ).toBe(true);
    expect(requests[0]?.headers.get("Upload-Metadata")).not.toMatch(
      /fileName|device|original/i,
    );
  ...
  it("keeps the resumable URL only for the same tab identity after an interrupted PATCH", async () => {
    ...
    await expect(
      new StorageUpload(grant("TUS"), identity, config, auth).send(
        blob,
        () => {},
      ),
    ).rejects.toThrow();
...
describe("capture and intake", () => {
  ...
  it("sends the completed Blob MIME/bytes and requests TUS only above 6 MiB", async () => {
    for (const size of [TUS_CHUNK_BYTES, TUS_CHUNK_BYTES + 1]) {
      ...
      const api = {
        POST: vi.fn(
          async (path: string, options: { body: Record<string, unknown> }) => {
            ...
            if (path === "/episodes/{episode_id}/uploads")
              throw new Error("stop after grant request");
            return { data: {}, response: { ok: true } };
          },
        ),
      } as unknown as RealApiClient;
      const session = new AudioIntake(
        "baby",
        api,
        config,
        auth,
      ...
      session.select(
        "FILE",
        new Blob([new Uint8Array(size)], { type: "audio/mp4" }),
        null,
        false,
      );
      await session.start();
      expect(body[1]).toMatchObject({
        mime_type: "audio/mp4",
        bytes: size,
        prefer_resumable: size > TUS_CHUNK_BYTES,
      });
      session.dispose();
    }
  });
  ...
  it("keeps CLIPPING as a READY warning and explains server rejection reasons", () => {
    const base = {
      ...
      rejection_code: null,
    } as components["schemas"]["AudioAsset"];
    ...
    expect(
      audioStatusMessage({
        ...base,
        status: "REJECTED",
        quality_reasons: ["TOO_SHORT", "SILENCE"],
      }),
    ).toContain("1초보다 짧아요");
  });
  ...
  it("recovers a lost complete response using the same logical request and server final state", async () => {
    ...
    const api = {
      POST: vi.fn(
        async (path: string, options: { body: Record<string, unknown> }) => {
          if (path === "/episodes")
            return { data: { episode_id: "episode" }, response: { ok: true } };
          if (path === "/episodes/{episode_id}/uploads")
            return {
              data: { audio, upload: grant("STANDARD") },
              response: { ok: true },
            };
      ...
      GET: vi.fn(async () => ({
        data: {
          audio_assets: [
            { ...audio, status: completeCount > 1 ? "READY" : "ALLOCATED" },
          ],
        },
        response: { ok: true },
      })),
    ...
    const session = new AudioIntake("baby", api, config, auth, scope, (view) =>
      views.push({ stage: view.stage, message: view.message }),
    );
          ...
          if (completeCount === 1)
            throw new ContractApiError(
              503,
              {
                ...
                message: "Temporary decoder failure",
                retryable: true,
                request_id: "request",
                field_errors: [],
                details: {},
  ...
  it("aborts transport before server cancellation, and uses same object key on expired renewal", async () => {
    ...
    const send = vi.spyOn(StorageUpload.prototype, "send").mockImplementation(
      () =>
        new Promise<void>((_resolve, reject) => {
          rejectTransfer = reject;
        }),
    );
    vi.spyOn(StorageUpload.prototype, "abort").mockImplementation(() => {
      events.push("abort");
    ...
    const api = {
      POST: vi.fn(async (path: string) => {
        ...
        if (path === "/uploads/{upload_id}/cancel") {
          events.push("cancel");
          return { data: audio, response: { ok: true } };
        }
        if (path === "/audio-assets/{audio_id}/uploads")
          return {
            data: {
              audio,
              upload: { ...grant("STANDARD"), upload_id: "new-upload" },
            },
            response: { ok: true },
          };
        throw new Error("unexpected mutation");
    ...
    const another = new AudioIntake("baby", api, config, auth, scope, (view) =>
      stages.push(view.stage),
    );
    ...
    (api.POST as ReturnType<typeof vi.fn>)
      ...
      .mockImplementationOnce(async () => ({
        data: { audio, upload: expiredGrant },
        response: { ok: true },
      }));
    await another.start();
  ...
  it("cleans an in-flight transfer when the baby or account scope changes", async () => {
    for (const [userId, babyId] of [
      ...
      const api = {
        POST: vi.fn(async (path: string) =>
          path === "/episodes"
            ...
            : { data: { audio, upload: grant("TUS") }, response: { ok: true } },
        ),
      } as unknown as RealApiClient;
      let started: (() => void) | null = null;
      vi.spyOn(StorageUpload.prototype, "send").mockImplementation(
        () =>
      ...
      const abort = vi
        .spyOn(StorageUpload.prototype, "abort")
        .mockImplementation(() => {
          started?.();
        });
      const session = new AudioIntake(
  ...
  it("checks current retention and clears downloaded playback on scope change", async () => {
      ...
      POST: vi.fn(async (path: string) =>
        path === "/episodes"
          ...
          : path === "/episodes/{episode_id}/uploads"
            ? {
                data: { audio: asset(), upload: grant("STANDARD") },
                response: { ok: true },
              }
          ...
          : path === "/consents"
            ...
            : {
                data: {
                  audio_id: "audio",
                  playback_url:
                    "https://project.supabase.co/storage/v1/object/sign/baby-audio/baby/audio/random?token=private",
    ...
    const create = vi
      ...
      .mockReturnValue("blob:private-playback");
    const revoke = vi
