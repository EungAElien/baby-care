...
import { AnalysisFlow, type AnalysisView } from "../src/lib/analysis/analysis-flow";
...
function analysisFor(id: string, overrides: Partial<Analysis> = {}): Analysis {
  return {
    analysis_id: id, baby_id: "baby", episode_id: "episode", audio_id: "audio", created_by_user_id: "user",
    status: "RUNNING", stage: "INFERENCE", attempt_no: 1, lease_expires_at: new Date(Date.now() + 60_000).toISOString(),
    quality_status: "PENDING", quality_reasons: [], cry_detected: null, audio_candidates: [], abstain_reason: null,
    failure: null, model_version: null, preprocess_version: null, label_mapping_version: null, context_snapshot: null,
    recommendation: null, inference_mode: "REAL", inference_executed: false, data_origin: "USER",
    recorded_at: new Date().toISOString(), completed_at: null,
    ...overrides,
  };
}
...
function completeFor(id: string, overrides: Partial<Analysis> = {}): Analysis {
  return analysisFor(id, {
    status: "COMPLETE", stage: "FINISHED", lease_expires_at: null, quality_status: "PASS", cry_detected: true,
    audio_candidates: [{ code: "hungry", label: "배고픔 신호", rank: 1 }],
    model_version: "v1", preprocess_version: "pre-v1", label_mapping_version: "label-v1",
    recommendation: {
      recommendation_id: "rec", analysis_id: id, context_snapshot_id: null, status: "GENERAL_CHECKLIST",
      actions: [], optional_questions: [], help_action: null, policy_version: "policy-v1", supersedes_id: null,
...
function abstainFor(id: string): Analysis {
  return analysisFor(id, {
    status: "ABSTAIN", stage: "FINISHED", lease_expires_at: null, quality_status: "INSUFFICIENT",
    cry_detected: false, audio_candidates: [], abstain_reason: "LOW_QUALITY", completed_at: new Date().toISOString(),
  });
}

function failedFor(id: string, attempt = 1): Analysis {
  return analysisFor(id, {
    status: "FAILED", stage: "FINISHED", lease_expires_at: null, attempt_no: attempt, completed_at: new Date().toISOString(),
    failure: { code: "INFERENCE_ERROR", message: "model crashed", retryable: true },
  });
}

function apiError(status: number, code: string, extra: Partial<Record<string, unknown>> = {}, headers = new Headers()): ContractApiError {
  return new ContractApiError(status, {
    code, message: "error", retryable: status === 429 || status === 503, request_id: "req",
    field_errors: [], details: {
      current_version: null, current_resource: null, resource_type: null,
      existing_analysis_id: null, existing_run_id: null, existing_session_id: null, deletion_job_id: null,
      session_revocation_id: null, reauthentication_challenge_id: null, status_url: null, retry_after_seconds: null,
      ...extra,
    },
  } as never, headers);
}
...
type PostOptions = { params: { path?: Record<string, string>; header: Record<string, string> }; body: Record<string, unknown> };
...
describe("A-06 ① analysis request/recovery", () => {
  it("sends analysis_id/client_request_id as the Idempotency-Key and body, and settles on the first COMPLETE response", async () => {
    ...
    const posts: Array<{ path: string; body: Record<string, unknown>; key: string }> = [];
    const api = {
      POST: vi.fn(async (path: string, options: PostOptions) => {
        posts.push({ path, body: options.body, key: options.params.header["Idempotency-Key"] ?? "" });
        return ok(completeFor(String(options.body.analysis_id)));
      }),
    } as unknown as RealApiClient;
    const flow = new AnalysisFlow("baby", "episode", "audio", api, scope, listener);
    flow.start();
    ...
    expect(posts[0]!.body.analysis_id).not.toBe(posts[0]!.body.client_request_id);
  ...
  it("recovers a lost create response with a GET on the same analysis_id, never a second POST", async () => {
    ...
    let getCount = 0;
    const api = {
      POST: vi.fn(async (_path: string, options: PostOptions) => { postCount++; sentId = String(options.body.analysis_id); throw new NetworkRequestError(); }),
      GET: vi.fn(async (_path: string, options: GetOptions) => { getCount++; expect(options.params.path.analysis_id).toBe(sentId); return ok(completeFor(sentId)); }),
    } as unknown as RealApiClient;
    const flow = new AnalysisFlow("baby", "episode", "audio", api, scope, listener);
    flow.start();
    await vi.runAllTimersAsync();
  ...
  it("stops waiting on the POST after 65s and switches to GET recovery without creating a new analysis", async () => {
    ...
    const api = {
      POST: vi.fn((_path: string, options: PostOptions) => { sentId = String(options.body.analysis_id); return pending.promise; }),
      GET: vi.fn(async () => { getCount++; return ok(analysisFor(sentId)); }),
    } as unknown as RealApiClient;
    const flow = new AnalysisFlow("baby", "episode", "audio", api, scope, listener);
    flow.start();
  ...
  it("polls RUNNING every 2 seconds and stops once a terminal status arrives", async () => {
    ...
    let getCount = 0;
    const api = {
      POST: vi.fn(async (_path: string, options: PostOptions) => { sentId = String(options.body.analysis_id); return ok(analysisFor(sentId)); }),
      GET: vi.fn(async () => { getCount++; return ok(getCount < 2 ? analysisFor(sentId) : completeFor(sentId)); }),
    } as unknown as RealApiClient;
    const flow = new AnalysisFlow("baby", "episode", "audio", api, scope, listener);
    flow.start();
    await vi.advanceTimersByTimeAsync(0);
  ...
  it("retries only a stored FAILED result with the same analysis_id, a fresh key and the current attempt_no", async () => {
    ...
    const api = {
      POST: vi.fn(async (path: string, options: PostOptions) => {
        ...
        if (path === "/episodes/{episode_id}/analyses") { sentId = String(options.body.analysis_id); return ok(failedFor(sentId, 1)); }
    ...
    expect(posts[1]!.body.client_request_id).not.toBe(createBody.client_request_id);
    expect(views.at(-1)?.analysis?.status).toBe("COMPLETE");
    flow.dispose();
  });
  ...
  it("refuses to retry a COMPLETE or ABSTAIN result", async () => {
    const { scope, views, listener } = setup();
    const api = {
      POST: vi.fn(async (_path: string, options: PostOptions) => ok(completeFor(String(options.body.analysis_id)))),
    } as unknown as RealApiClient;
    const flow = new AnalysisFlow("baby", "episode", "audio", api, scope, listener);
    flow.start();
    await vi.runAllTimersAsync();
    ...
    const abstainApi = {
      POST: vi.fn(async (_path: string, options: PostOptions) =>
        ok({ ...abstainFor(String(options.body.analysis_id)), episode_id: "episode2", audio_id: "audio2" })),
    } as unknown as RealApiClient;
    const abstainFlow = new AnalysisFlow("baby", "episode2", "audio2", abstainApi, scope, listener);
    abstainFlow.start();
    await vi.runAllTimersAsync();
  ...
  it("blocks on MODEL_NOT_READY without inventing a result, and does not enable a new analysis until the user resumes", async () => {
    ...
    const api = {
      POST: vi.fn(async (_path: string, options: PostOptions) => {
        calls++;
        if (calls === 1) throw apiError(503, "MODEL_NOT_READY", { retry_after_seconds: 5 }, new Headers({ "Retry-After": "5" }));
        return ok(completeFor(String(options.body.analysis_id)));
      }),
    } as unknown as RealApiClient;
    const flow = new AnalysisFlow("baby", "episode", "audio", api, scope, listener);
  ...
  it("blocks on 429 with the server's Retry-After and only continues when the user resumes", async () => {
    ...
    const api = {
      POST: vi.fn(async (_path: string, options: PostOptions) => {
        calls++;
        if (calls === 1) throw apiError(429, "RATE_LIMITED", { retry_after_seconds: 3 }, new Headers({ "Retry-After": "3" }));
        return ok(completeFor(String(options.body.analysis_id)));
      }),
    } as unknown as RealApiClient;
    const flow = new AnalysisFlow("baby", "episode", "audio", api, scope, listener);
  ...
  it("recovers ANALYSIS_IN_PROGRESS with the server-supplied existing analysis_id instead of creating a new one", async () => {
    ...
    const api = {
      POST: vi.fn(async (path: string) => {
        posts.push(path);
        throw apiError(409, "ANALYSIS_IN_PROGRESS", { existing_analysis_id: "existing-analysis" });
      }),
    ...
    expect(api.GET).toHaveBeenCalledWith("/analyses/{analysis_id}", { params: { path: { analysis_id: "existing-analysis" } } });
    expect(views.at(-1)?.phase).toBe("resolved");
  ...
  it("does not auto-retry an IDEMPOTENCY_KEY_REUSED mismatch into a new analysis", async () => {
    const { scope, views, listener } = setup();
    const api = { POST: vi.fn(async () => { throw apiError(409, "IDEMPOTENCY_KEY_REUSED"); }) } as unknown as RealApiClient;
    const flow = new AnalysisFlow("baby", "episode", "audio", api, scope, listener);
    flow.start();
    await vi.runAllTimersAsync();
  ...
  it("surfaces a permission error without a retry affordance", async () => {
    const { scope, views, listener } = setup();
    const api = { POST: vi.fn(async () => { throw apiError(403, "CONSENT_REQUIRED"); }) } as unknown as RealApiClient;
    const flow = new AnalysisFlow("baby", "episode", "audio", api, scope, listener);
    flow.start();
    await vi.runAllTimersAsync();
    expect(views.at(-1)?.phase).toBe("error");
