...
export function BabyProfile({
  baby,
  isOwner,
}: Readonly<{ baby: Baby; isOwner: boolean }>) {
  ...
  async function save(event: React.FormEvent) {
    ...
    if (
      Array.from(draft.alias.trim()).length < 1 ||
      Array.from(draft.alias.trim()).length > 40
    ) {
      setError("아기 이름은 1~40자로 입력해 주세요.");
      return;
    }
    try {
      new Intl.DateTimeFormat("ko-KR", { timeZone: draft.timezone });
    } catch {
      setError("올바른 시간대를 입력해 주세요. 예: Asia/Seoul");
      return;
    }
    ...
    const body = uncertain ?? {
      ...
      feeding_mode: draft.feeding_mode,
    ...
    try {
      ...
      setSaved(true);
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: babiesKey(captured.userId) }),
        queryClient.invalidateQueries({
          queryKey: activeBabyKey(captured.userId),
        }),
      ]);
    } catch (failure) {
      if (!active.current || scope.snapshot() !== captured) return;
      if (
        failure instanceof ContractApiError &&
        failure.envelope.code === "VERSION_CONFLICT"
      ) {
        void queryClient.invalidateQueries({
          queryKey: babiesKey(captured.userId),
        });
        setError(
          "다른 보호자가 프로필을 변경했어요. 최신 값을 확인한 뒤 다시 수정해 주세요.",
        );
        try {
          const refreshed = requireData(
            await client.GET("/babies", {}),
          ).items.find((item) => item.baby.baby_id === baby.baby_id);
          scope.assertCurrent(captured);
          if (active.current && refreshed) setLatest(refreshed.baby);
        } catch {
          if (active.current && scope.snapshot() === captured)
            setError(
              "최신 정보를 불러오지 못했어요. 창을 닫고 다시 열어 확인해 주세요.",
            );
        }
      } else if (isCareEventOutcomeUnknown(failure)) {
        setUncertain(body);
        setError(
          "저장 응답을 받지 못했어요. 입력을 유지한 채 같은 요청으로 결과를 확인해 주세요.",
        );
      } else
        setError(
          failure instanceof ContractApiError
    ...
    <ScreenSection title="아기 프로필">
      <div className="flex items-center gap-3">
        <span className="brand-symbol">
          <BabyIcon aria-hidden="true" />
        </span>
        <div>
          <p className="text-xl font-bold">{baby.alias}</p>
      ...
      <List>
        <ListItem title="생일" detail={baby.birth_date} />
        <ListItem title="수유 방식" detail={feedingLabel[baby.feeding_mode]} />
        <ListItem title="기록 시간대" detail={baby.timezone} />
      </List>
      {saved && <p role="status">프로필을 저장했어요.</p>}
      {isOwner && (
        <BottomSheetRoot open={open} onOpenChange={changeOpen}>
          ...
          <BottomSheetContent
            title="아기 프로필 수정"
            description="함께 돌보는 보호자에게도 변경된 정보가 보여요."
            showCloseButton={!busy}
          >
            <form onSubmit={save} className="flex min-h-0 flex-col">
              <BottomSheetBody>
                <fieldset
                  disabled={busy || uncertain !== null}
                ...
                >
                  <TextField
                    ...
                    value={draft.alias}
                    onValueChange={({ value }) =>
                      setDraft({ ...draft, alias: value })
                    }
                  >
                    <TextFieldInput required autoComplete="off" />
                  </TextField>
                  <TextField
                    label="생일"
                    value={draft.birth_date}
                    onValueChange={({ value }) =>
                      setDraft({ ...draft, birth_date: value })
                    }
                  >
                  ...
                  <label className="flex flex-col gap-2">
                    ...
                    <select
                      ...
                      value={draft.feeding_mode}
                      onChange={(e) =>
                        setDraft({
                          ...draft,
                          feeding_mode: e.target.value as Baby["feeding_mode"],
                        })
                      }
                    >
                      {Object.entries(feedingLabel).map(([value, label]) => (
                ...
                {latest && (
                  <div className="status-note mt-4">
                    <p>
                      최신 프로필: {latest.alias} · {latest.birth_date} ·{" "}
                      {feedingLabel[latest.feeding_mode]}
                    </p>
                    <ActionButton
                      type="button"
                      variant="neutralWeak"
              ...
              <BottomSheetFooter>
                <ActionButton
                  type="submit"
                  loading={busy}
                  disabled={busy || latest !== null}
                >
                  {uncertain ? "같은 요청으로 결과 확인" : "변경 저장"}
                </ActionButton>
              </BottomSheetFooter>
            </form>
          </BottomSheetContent>
