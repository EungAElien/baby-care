...
export function ResolvedAnalysis({
  ...
  if (analysis.status === "COMPLETE") {
    return (
      <ScreenSection title="분석 완료 · 가능성 있는 원인"><div className="result-banner"><CheckCircle2 aria-hidden="true" /><p>소리에서 살펴본 후보예요. 실제 원인일 확률이나 확정 진단을 뜻하지 않아요.</p></div>
  ...
  if (analysis.status === "ABSTAIN") {
    return (
      <ScreenSection title="판단을 유보했어요"><div className="result-banner"><CircleHelp aria-hidden="true" /><p>충분한 근거를 얻지 못했어요. 아기의 모습을 직접 확인해 주세요.</p></div>
  ...
  return (
    <ScreenSection title="분석을 완료하지 못했어요">
      ...
      {failure?.retryable && (
        <ActionButton variant="neutralWeak" type="button" onClick={onRetryFailed} className="flex min-h-11 items-center justify-center rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground">
    ...
    <div className="flex flex-col gap-4">
      {view.analysis && (
        <div className="flex items-center justify-between">
          <SourceBadge inferenceMode={view.analysis.inference_mode} dataOrigin={view.analysis.data_origin} inferenceExecuted={view.analysis.inference_executed} />
      ...
      {view.phase === "blocked" && (
        <ScreenSection title="지금은 진행할 수 없어요">
          ...
          <div className="flex gap-2">
            <ActionButton variant="neutralWeak" type="button" onClick={() => flow.current?.resume()} className="flex min-h-11 items-center rounded-md border border-border px-4 text-sm text-foreground">
              다시 시도
            </ActionButton>
            <Link href={`/babies/${babyId}/quick-record`} className="flex min-h-11 items-center rounded-md border border-border px-4 text-sm text-foreground">
              직접 기록하기
