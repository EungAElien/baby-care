...
import {
  ...
  type Analysis,
  type AnalysisView,
} from "@/lib/analysis/analysis-flow";
import {
  abstainReasonLabel,
  failureCodeLabel,
  qualityReasonLabel,
...
function VersionInfo({ analysis }: Readonly<{ analysis: Analysis }>) {
  return (
    <div className="flex flex-col gap-1 border-t border-border pt-2 text-xs text-muted-foreground">
      <p>
        모델 {analysis.model_version ?? "알 수 없음"} · 전처리{" "}
        {analysis.preprocess_version ?? "알 수 없음"} · 라벨 매핑{" "}
        {analysis.label_mapping_version ?? "알 수 없음"}
      </p>
      ...
      {analysis.quality_reasons.length > 0 && (
        <p>
          ...
          {analysis.quality_reasons
            .map((reason) => qualityReasonLabel[reason] ?? reason)
            .join(", ")}
        </p>
      )}
    </div>
...
export function ResolvedAnalysis({
  ...
  if (analysis.status === "COMPLETE") {
    return (
      <ScreenSection title="분석 완료 · 가능성 있는 원인">
        <div className="result-banner">
          <CheckCircle2 aria-hidden="true" />
          <p>
            소리에서 살펴본 후보예요. 실제 원인일 확률이나 확정 진단을 뜻하지
            않아요.
          </p>
        </div>
        <ul className="flex flex-col gap-2">
          {analysis.audio_candidates.slice(0, 3).map((candidate) => (
            ...
            >
              {candidate.label}{" "}
              <span className="text-xs text-muted-foreground">가능성</span>
            </li>
          ))}
        </ul>
        {analysis.recommendation &&
          analysis.recommendation.status !== "HELP_REQUIRED" && (
        ...
        {analysis.recommendation?.status === "HELP_REQUIRED" &&
          analysis.recommendation.help_action && (
            <p className="text-sm text-foreground">
              {analysis.recommendation.help_action.text}
            </p>
          )}
    ...
    return (
      <ScreenSection title="판단을 유보했어요">
        <div className="result-banner">
          ...
          <p>충분한 근거를 얻지 못했어요. 아기의 모습을 직접 확인해 주세요.</p>
        </div>
        <p className="text-sm text-foreground">
          {analysis.abstain_reason
            ? (abstainReasonLabel[analysis.abstain_reason] ??
              analysis.abstain_reason)
            : "사유 없음"}
        </p>
  ...
  return (
    <ScreenSection title="분석을 완료하지 못했어요">
      <ErrorState
        label={
          failure
            ? (failureCodeLabel[failure.code] ?? failure.message)
            : "알 수 없는 오류"
        }
...
export function AnalysisScreen({
  ...
  useEffect(() => {
    ...
    const instance = new AnalysisFlow(
      babyId,
      episodeId,
      audioId,
      client,
    ...
    return () => {
      unregister();
      cleanup();
      if (flow.current === instance) flow.current = null;
    };
  ...
  if (!available) {
    return (
      <p className="text-sm text-muted-foreground">
        실제 분석은 로그인과 서버 연결이 있을 때만 시작할 수 있어요.
      </p>
    );
  }

  if (capabilities.isLoading)
    return <LoadingState label="분석 가능 여부를 확인하고 있어요" />;
  if (capabilities.isError)
    return <ErrorState label="분석 가능 여부를 확인하지 못했어요." retryable />;
    ...
    <div className="flex flex-col gap-4">
      {view.analysis && (
        <div className="flex items-center justify-between">
          <SourceBadge
            inferenceMode={view.analysis.inference_mode}
            dataOrigin={view.analysis.data_origin}
            inferenceExecuted={view.analysis.inference_executed}
          />
          <span className="text-xs text-muted-foreground">
            시도 {view.analysis.attempt_no}회차
          </span>
        </div>
      )}
      ...
      {view.phase === "idle" && !gateOpen && (
        <ScreenSection title="아직 분석을 시작할 수 없어요">
          <p className="text-sm text-foreground">
            서버가 이 모델을 아직 제품 분석에 사용할 준비를 마치지 않았어요
            (MODEL_NOT_READY).
          </p>
          <p className="text-xs text-muted-foreground">
            장애가 아니라 준비 상태이며, 실제 API 장애를 목 결과로 대체하지
            않아요.
          </p>
          <div className="flex gap-2">
            <Link
      ...
      {(view.phase === "requesting" ||
        ...
        view.phase === "polling") && <LoadingState label={view.message} />}
      ...
      {view.phase === "blocked" && (
        <ScreenSection title="지금은 진행할 수 없어요">
          ...
          {view.retryAfterSeconds !== null && (
            <p className="text-xs text-muted-foreground">
              서버가 안내한 대기 시간: {view.retryAfterSeconds}초
            </p>
          )}
          <div className="flex gap-2">
            <ActionButton
      ...
      {view.phase === "error" &&
        view.errorKind !== "permission" &&
        view.errorKind !== "not-found" && (
      ...
      {view.phase === "resolved" && view.analysis && (
        <ResolvedAnalysis
          ...
          onRetryFailed={() => flow.current?.retryFailed()}
        />
      )}
    </div>
  );
