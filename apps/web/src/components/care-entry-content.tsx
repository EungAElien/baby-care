...
export function CareEntryContent({ content, onChange, disabled, fields }: {
  ...
  return <fieldset disabled={disabled} className="flex flex-col gap-4">
    ...
    {content.actions.map((item, index) => <section aria-label={`행동 ${index + 1}`} className="grid gap-2 rounded-lg border p-3" key={item.action_ref}>
      ...
      <button type="button" className={entryControl} onClick={() => onChange({ ...content, actions: content.actions.filter((_, position) => position !== index) })}>이 행동 제외</button>
    </section>)}
    <button type="button" className={entryControl} onClick={() => onChange({ ...content, actions: [...content.actions, {
    ...
    {content.states.map((item, index) => <section aria-label={`상태 관찰 ${index + 1}`} key={index} className="grid gap-2 rounded-lg border p-3">
      ...
      <button type="button" className={entryControl} onClick={() => onChange({ ...content, states: content.states.filter((_, position) => position !== index) })}>이 관찰 제외</button>
    </section>)}
    <button type="button" className={entryControl} onClick={() => onChange({ ...content, states: [...content.states, {
    ...
    {content.caregiver_interpretations.map((item, index) => <section key={index} className="grid gap-2 rounded-lg border p-3">
      ...
      <button type="button" className={entryControl} onClick={() => onChange({ ...content, caregiver_interpretations: content.caregiver_interpretations.filter((_, position) => position !== index) })}>이 해석 제외</button>
    </section>)}
    <button type="button" className={entryControl} onClick={() => onChange({ ...content, caregiver_interpretations: [...content.caregiver_interpretations, { text: "", certainty: "CAREGIVER_REPORTED", evidence: [] }] })}>해석 직접 추가</button>
    {content.outcomes.length > 0 && <section className="grid gap-2 rounded-lg border p-3">
      ...
      <button type="button" className={entryControl} onClick={() => onChange({ ...content, outcomes: [] })}>반응 후보 제외</button>
    </section>}
    ...
    {content.unresolved.map((item, index) => <section key={`${item.field}-${index}`} className="grid gap-2 rounded-lg border p-3">
      ...
      <button type="button" className={entryControl} onClick={() => onChange({ ...content, unresolved: content.unresolved.filter((_, position) => position !== index) })}>관련 카드를 수정했고 이 항목을 해소했어요</button>
    </section>)}
  </fieldset>;
}
