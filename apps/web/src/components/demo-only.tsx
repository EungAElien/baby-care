...
export function DemoOnly({ children, fallback = null }: Readonly<{ children: React.ReactNode; fallback?: React.ReactNode }>) {
  const real = useRealSession();
  return isMockNavEnabled() && real.status === "signed-out" ? children : fallback;
}
