...
import {
  CareEventCard,
  CareEventValueSummary,
} from "@/components/care-event-card";
...
import {
  getCareEvent,
  getCareEventAfterConflict,
  usePatchCareEventMutation,
} from "@/lib/api/care-events";
...
function editErrorMessage(error: unknown): string {
  if (error instanceof ContractApiError) {
    switch (error.envelope.code) {
      case "AUTHOR_ONLY":
        return "이 기록을 수정할 권한이 없어요.";
      case "RESOURCE_NOT_FOUND":
        return "찾을 수 없거나 접근할 수 없어요.";
      case "SLEEP_ALREADY_ACTIVE":
        return "진행 중인 다른 수면 기록이 있어요. 타임라인에서 확인해 주세요.";
      case "VALIDATION_ERROR":
        return "입력값을 확인해 주세요.";
      case "IDEMPOTENCY_KEY_REUSED":
        return "같은 요청 키에 다른 내용이 사용됐어요. 자동으로 다시 보내지 않았어요.";
      default:
        return error.envelope.message;
    }
  }
  if (isCareEventOutcomeUnknown(error))
    return "수정 결과를 확인하지 못했어요. 같은 요청으로 결과를 다시 확인해 주세요.";
...
export function CareEventEdit({
  ...
  const [pendingRequest, setPendingRequest] =
    useState<PatchCareEventRequest | null>(null);
  ...
  useEffect(() => {
    setCareEventDirty(
      babyId,
      form.formState.isDirty || pendingRequest !== null || conflict !== null,
    );
  }, [
    babyId,
    conflict,
    ...
    try {
      const latest =
        error === undefined
          ...
          : await getCareEventAfterConflict(
              client,
              babyId,
              baseline.care_event_id,
              error,
            );
      ...
      if (
        readError instanceof ContractApiError &&
        (readError.status === 404 || readError.status === 403)
      ) {
        onAccessLost();
        return;
      }
  ...
  async function send(request: PatchCareEventRequest) {
    ...
    try {
      ...
      if (
        ...
        error.status === 409 &&
        error.envelope.code === "VERSION_CONFLICT"
      ) {
        setPendingRequest(null);
    ...
    <div className="flex flex-col gap-4">
      <ScreenSection title="기록 수정">
        <p className="text-xs text-muted-foreground">
          읽은 version {baseVersion}을 기준으로 저장해요. 다른 보호자의 변경을
          자동으로 합치지 않아요.
        </p>
        <form onSubmit={submit} className="mt-3 flex flex-col gap-4" noValidate>
          <CareEventFields
            form={form}
            disabled={
              mutation.isPending || pendingRequest !== null || conflict !== null
            }
          />
          <ActionButton
            type="submit"
      ...
      {saveError !== null && (
        <div className="flex flex-col gap-2">
          <ErrorState
            label={editErrorMessage(saveError)}
            retryable={pendingRequest !== null}
          />
          {pendingRequest && (
            <ActionButton
              variant="neutralWeak"
      ...
      {conflict && (
        <div className="flex flex-col gap-3" role="alert">
          <p className="text-sm font-medium text-destructive">
            다른 변경이 먼저 저장됐어요. 자동 덮어쓰기는 하지 않았어요.
          </p>
          <ScreenSection
            title={`내 미저장 수정안 · 읽은 version ${conflict.attemptedVersion}`}
          >
          ...
          {conflict.latest ? (
            <>
              ...
              <p className="text-xs text-muted-foreground">
                마지막 선택은 최신 기록을 내 수정안으로 대체합니다. 두 입력을
                합치지는 않아요.
              </p>
            </>
          ) : (
            <div className="flex flex-col gap-2">
              <p className="text-sm text-muted-foreground">
                {conflict.readFailed
                  ? "최신 기록을 다시 조회하지 못했어요. 409 응답에 담긴 값은 표시하지 않습니다."
                  : "현재 권한으로 최신 기록을 다시 확인하고 있어요."}
              </p>
              {conflict.readFailed && (
                <ActionButton
                  variant="neutralWeak"
                  type="button"
                  onClick={() =>
                    void readLatest(conflict.draft, conflict.attemptedVersion)
                  }
                  className="min-h-11 rounded-md border border-border px-4 text-sm"
                >
                  최신 기록 다시 조회
