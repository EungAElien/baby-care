...
export function ConsentPanel({
  ...
  async function grant(
    ...
    setMessage(null);
    try {
      if (scope === "CONTRIBUTOR_TRAINING")
        await setMyTrainingConsent(
          client,
          babyId,
          true,
          policyVersion,
      ...
      setMessage(
        error instanceof ContractApiError && error.kind === "version-conflict"
  ...
  const personal = latestConsent(
    rows,
    "CONTRIBUTOR_TRAINING",
    real.userId ?? undefined,
  );
    ...
    <ScreenSection title="동의와 보관">
        ...
        <>
          {babyScopes.map((scope) => {
            ...
            return (
              <div
                key={scope}
                className="flex flex-col gap-1 border-b border-border py-2 text-sm"
              >
                <span className="font-medium text-foreground">
                  {labels[scope]}
                </span>
                <span className="text-muted-foreground">
                  {current?.status ?? "NOT_GRANTED"}
                </span>
                ...
                {isOwner &&
                  ...
                  scope !== "BABY_TRAINING" && (
                    <GrantControl
                      scope={scope}
                      disabled={busy}
                      onGrant={(policyVersion, requestId) =>
                        grant(
                          scope,
                          current?.version ?? 0,
                          policyVersion,
          ...
          <div className="flex flex-col gap-1 border-b border-border py-2 text-sm">
            <span className="font-medium text-foreground">
              {labels.CONTRIBUTOR_TRAINING}
            </span>
            <span className="text-muted-foreground">
              {personal?.status ?? "NOT_GRANTED"}
            </span>
          ...
          <p className="text-sm text-muted-foreground">
            {labels.SHARED_USE}: {sharedUse?.status ?? "NOT_GRANTED"}
          </p>
          <p className="text-xs text-muted-foreground">
            새 동의는 승인된 안내 문구와 정책 버전을 확인한 뒤 받을 수 있어요.
            이메일 인증이나 공동 사용 수락은 학습 동의가 아니에요.
          </p>
        </>
      )}
      {pending && (
      ...
      {trainingRequest && (
        <ReauthenticationPanel
          key={trainingRequest.requestId}
          babyId={babyId}
          operation="ENABLE_BABY_TRAINING"
          title="아기 자료 학습 참여 재인증"
          onProof={async (proofToken) => {
            ...
            await setBabyConsent(
              ...
              "BABY_TRAINING",
              true,
              trainingRequest.policyVersion,
              trainingRequest.version,
              trainingRequest.requestId,
              proofToken,
            );
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <ActionButton
        ...
        onClick={() => {
          void onGrant(policy.version, requestId).then(
            () => setRetry(false),
            () => setRetry(true),
          );
        }}
      ...
      {retry && (
        <ActionButton
          ...
          className="min-h-11 w-fit text-xs"
        >
          새 요청으로 시작
        </ActionButton>
      )}
