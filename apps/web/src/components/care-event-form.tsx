  ...
  if (error instanceof ContractApiError) {
    switch (error.envelope.code) {
      case "SLEEP_ALREADY_ACTIVE":
        return "진행 중인 수면 기록이 있어요. 타임라인에서 확인해 주세요.";
      ...
      case "IDEMPOTENCY_KEY_REUSED":
        return "이 요청의 중복 방지 키가 다른 내용에 사용됐어요. 자동 재전송하지 않았어요.";
      default:
        return error.envelope.message;
    }
  }
  if (isCareEventOutcomeUnknown(error))
    return "저장 결과를 확인하지 못했어요. 같은 요청으로 다시 확인하거나 타임라인을 살펴봐 주세요.";
...
function activeSleepId(error: unknown, babyId: string): string | null {
  if (
    ...
    error.envelope.code !== "SLEEP_ALREADY_ACTIVE"
  )
    return null;
  ...
  if (
    ...
    typeof record.care_event_id !== "string" ||
    typeof event !== "object" ||
    event === null ||
    Array.isArray(event) ||
    (event as Record<string, unknown>).type !== "SLEEP"
  )
    return null;
    ...
    <div className="flex flex-col gap-4">
      <ScreenSection title="선택지로 생활 기록">
        <form onSubmit={submit} className="flex flex-col gap-4" noValidate>
          <CareEventFields
            form={form}
            disabled={mutation.isPending || pendingRequest !== null}
          />

          <p className="text-xs text-muted-foreground">
            실제로 한 일만 저장해 주세요. 선택한 종류와 값 외의 정보를 자동으로
            추정하지 않아요.
          </p>
          <ActionButton
            type="submit"
          ...
          {!canSave && (
            <p className="text-xs text-muted-foreground">
              이 화면은 계약 예시 미리보기예요. 실제 저장은 로그인한 API
              환경에서만 할 수 있어요.
            </p>
          )}
        </form>
      ...
      {saveError !== null && (
        <div className="flex flex-col gap-2">
          <ErrorState
            label={saveErrorMessage(saveError)}
            retryable={pendingRequest !== null}
          />
          {activeSleepId(saveError, babyId) && (
            <Link
              href={`/babies/${babyId}/care-events/${activeSleepId(saveError, babyId)}`}
      ...
      {savedEvent && (
        <CareEventCard event={savedEvent} babyId={babyId} linkToDetail />
      )}
    </div>
  );
}
