...
export function AudioIntakeScreen({ babyId }: Readonly<{ babyId: string }>) {
  ...
  const available =
    ...
    client !== null &&
    config !== null &&
    scopeSnapshot.userId === real.userId &&
    scopeSnapshot.babyId === babyId;

  const capabilities = useCapabilitiesQuery(
    available && view.stage === "ready",
  );
  ...
  async function startRecording() {
    ...
    try {
      ...
      if (recorder.current === capture && intake.current === session)
        session.selectRecording(result, preferResumable);
    } catch (error) {
      if (recorder.current === capture && intake.current === session)
        setLocalError(
          error instanceof Error ? error.message : "녹음에 실패했어요.",
        );
    } finally {
  ...
  async function selectFile(file: File | undefined) {
    if (!file || recorder.current || operation.current || !intake.current)
      return;
    ...
    try {
      ...
      if (intake.current === session && selection.current === current)
        session.select("FILE", file, duration, preferResumable);
    } catch (error) {
      if (intake.current === session && selection.current === current)
        setLocalError(
          error instanceof Error ? error.message : "파일을 선택하지 못했어요.",
        );
    } finally {
      if (intake.current === session && selection.current === current) {
        operation.current = false;
  ...
  async function cancel() {
    ...
    try {
      ...
      if (intake.current === session)
        setLocalError(
          "취소 결과를 확인하지 못했어요. 같은 요청으로 다시 확인해 주세요.",
        );
    } finally {
      if (intake.current === session) cancellation.current = false;
    }
  }
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {!available && (
        <Callout
          prefixIcon={<Mic />}
          title="로그인 후 사용할 수 있어요"
          description="현재 화면에서는 녹음과 전송이 시작되지 않아요. 실제 계정과 서버 연결이 필요해요."
        />
      )}
      <div className="capture-panel" role="status" aria-live="polite">
        ...
        <p className="text-sm text-muted-foreground">
          {captureState === "requesting"
            ? "브라우저의 마이크 권한 요청을 확인해 주세요."
            : captureState === "recording"
              ? "최대 30초 뒤 자동으로 녹음이 끝나요."
              : "입력한 소리만 전송해요. 자동 감지는 아직 시작되지 않았어요."}
        </p>
      </div>
      <div className="flex flex-col gap-3 sm:flex-row">
        {!recording ? (
          ...
          >
            ...
            {captureState === "requesting" ? "권한 요청 취소" : "녹음 끝내기"}
          </ActionButton>
        )}
        <ActionButton
          variant="neutralWeak"
      ...
      <details>
        ...
        <label className="flex min-h-11 items-center gap-3 text-sm">
          <input
            type="checkbox"
            checked={preferResumable}
            disabled={!canChoose || view.stage !== "idle"}
            onChange={(event) => setPreferResumable(event.target.checked)}
          />
          끊긴 지점에서 재개할 수 있는 전송 사용
        </label>
      </details>
      {localError && <ErrorState label={localError} />}
      {view.stage !== "idle" && (
        <div role="status" aria-live="polite" className="status-note">
          <p>{view.message}</p>
          {view.source === "FILE" && (
            <p className="mt-2 text-sm text-muted-foreground">
              녹음 시각을 모르는 파일은 아기의 현재 상태와 연결해 해석하기
              어려워요.
            </p>
          )}
          {view.stage === "transferring" && (
            <progress
              aria-label="업로드 진행률"
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {view.grant && !terminal && (
          <ActionButton
            variant="ghost"
            disabled={view.stage === "cancelling"}
            onClick={() => void cancel()}
          >
            전송 중단·취소
          </ActionButton>
      ...
      {view.stage === "ready" && (
        <div className="flex flex-col gap-3">
          <Callout
            title="음원 준비 완료"
            description="분석을 시작하면 이 음원으로 아기의 신호를 살펴봐요."
          />
          {capabilities.isLoading && (
            <p role="status">분석 가능 여부를 확인하고 있어요.</p>
          )}
          {capabilities.isError && (
            <>
              <ErrorState label="분석 가능 여부를 확인하지 못했어요." />
              <ActionButton
                variant="neutralWeak"
          ...
          {capabilities.data &&
            ...
            (capabilities.data.audio_model.available ? (
              ...
              <Callout
                title="분석 준비 중"
                description="서버에서 아직 모델을 준비하지 못했어요. 직접 기록은 계속할 수 있어요."
              />
            ))}
          <ActionButton
            variant="neutralWeak"
            ...
            onClick={() =>
              void run(async (s) => {
                if (player.current) await s.play(player.current);
              })
            }
          >
            보관 동의 확인 후 재생
          ...
          <audio
            ...
            aria-label="등록한 음원 재생"
            onEnded={() => intake.current?.clearPlayback()}
          />
        </div>
      )}
    </div>
