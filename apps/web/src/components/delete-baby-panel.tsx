...
export function DeleteBabyPanel({
  baby,
}: Readonly<{ baby: components["schemas"]["Baby"] }>) {
  ...
  return (
    <ScreenSection title="아기 전체 자료 삭제">
      <p className="text-sm text-muted-foreground">
        삭제 요청을 받으면 공유 자료 접근이 즉시 차단됩니다. 정리 완료 여부는
        별도 작업 화면에서 확인합니다.
      </p>
      <label className="flex gap-2 text-sm">
        ...
        />
        이 아기의 전체 자료 삭제를 요청하겠습니다.
      </label>
      <ActionButton
        variant="neutralWeak"
        type="button"
        disabled={!confirm || !!intent || !client}
        onClick={() => {
          setError(null);
      ...
      {intent && (
        <ReauthenticationPanel
          ...
          onProof={async (proofToken) => {
            ...
            try {
              const job = await deleteBabyData(
                client,
                baby.baby_id,
                real.userId,
                intent.version,
              ...
              if (
                ...
                failure.kind === "version-conflict"
              ) {
                setIntent(null);
                setConfirm(false);
                setError(
                  "아기 정보 버전이 변경됐어요. 최신 설정을 다시 열어 확인한 뒤 새 요청을 시작해 주세요.",
                );
              }
              throw failure;
            }
