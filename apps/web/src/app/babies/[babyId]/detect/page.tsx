...
const examples = [
  ...
  { scenario: "analysis_complete_stub", label: "완료" },
  { scenario: "analysis_abstain", label: "판단 유보" },
  { scenario: "analysis_no_cry", label: "울음 미확인" },
  { scenario: "analysis_failed", label: "처리 실패" },
];

export default async function DetectPage({
  params,
}: Readonly<{ params: Promise<{ babyId: string }> }>) {
  const { babyId } = await params;
  return (
    <div className="flex flex-col gap-6">
      <ActionButton variant="ghost" size="small" asChild className="self-start">
      ...
      <DemoOnly>
        <details className="preview-tools">
          <summary>DEMO · 결과 상태 살펴보기</summary>
          <div className="mt-3 flex flex-wrap gap-2">
            {examples.map((example) => (
              <ActionButton
                ...
                asChild
              >
                <Link href={`/babies/${babyId}/results/${example.scenario}`}>
                  {example.label}
                </Link>
              </ActionButton>
            ))}
