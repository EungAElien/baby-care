...
export function supportedRecorderTypes(): string[] {
  ...
  return recorderCandidates.filter((candidate) =>
    MediaRecorder.isTypeSupported(candidate),
  );
}

export type FinishedRecording = Readonly<{
  blob: Blob;
  mimeType: string;
...
export class DirectRecording {
  ...
  constructor(
    private readonly onState?: (state: "requesting" | "recording") => void,
  ) {}
  ...
  async start(): Promise<FinishedRecording> {
    ...
    if (supported.length === 0)
      throw new Error("지원되는 녹음 형식을 찾지 못했어요.");
    if (document.hidden)
      throw new Error("이 화면을 다시 연 뒤 녹음을 시작해 주세요.");
    if (navigator.onLine === false)
      throw new Error("연결을 확인한 뒤 녹음을 시작해 주세요.");
    this.onState?.("requesting");
    const hidden = () => {
      if (document.hidden)
        this.abort(
          "화면이 숨겨져 녹음을 중단했어요. 화면을 연 뒤 다시 녹음해 주세요.",
        );
    };
    const offline = () =>
      this.abort(
        "연결이 끊겨 녹음을 중단했어요. 연결을 확인한 뒤 다시 녹음해 주세요.",
      );
    const pageHide = () =>
      this.abort(
        "화면을 떠나 녹음을 중단했어요. 다시 녹음하려면 직접 시작해 주세요.",
      );
    ...
    try {
      ...
      if (error instanceof DOMException && error.name === "NotAllowedError")
        throw new Error(
          "마이크 권한이 거부됐어요. 브라우저의 사이트 권한에서 마이크를 허용한 뒤 다시 시작하거나 파일을 선택해 주세요.",
        );
      throw error;
    } finally {
    ...
    try {
      ...
      recorder.ondataavailable = (event) => {
        if (event.data.size > 0) this.pieces.push(event.data);
      };
      recorder.onerror = () =>
        this.abort(
          "마이크 오류로 녹음을 중단했어요. 장치를 확인한 뒤 다시 녹음해 주세요.",
        );
      for (const track of stream.getTracks()) {
        track.onended = () =>
          this.abort(
            "마이크 연결이 끝났어요. 장치를 연결한 뒤 다시 녹음해 주세요.",
          );
        track.onmute = () =>
          this.abort(
            "마이크 입력이 중단됐어요. 장치를 확인한 뒤 다시 녹음해 주세요.",
          );
      }
      recorder.onstop = () => {
        this.clearTimer();
