...
import { FILE_MAX_SECONDS, MANUAL_MAX_SECONDS, MAX_AUDIO_BYTES, TUS_CHUNK_BYTES, type FinishedRecording } from "./recording";
...
type Stage = "idle" | "selected" | "creating" | "granting" | "transferring" | "interrupted" | "completing" | "uncertain" | "ready" | "rejected" | "expired" | "cancelling" | "cancelled" | "error";
...
const reasonText: Record<string, string> = {
  ...
  UNSUPPORTED_CODEC: "실제 음원 형식을 지원하지 않아요. 지원되는 원본 파일을 선택해 주세요.",
  DECODE_ERROR: "음원을 읽을 수 없어요. 손상되지 않은 다른 파일을 선택해 주세요.",
};

export function audioStatusMessage(audio: Asset): string {
  if (audio.status === "READY") {
    const warning = audio.quality_reasons.filter((reason) => reason === "CLIPPING").map((reason) => reasonText[reason]).join(" ");
    return `업로드·검증 완료. 분석은 아직 시작되지 않았어요.${warning ? ` 경고: ${warning}` : ""}`;
  }
  if (audio.status === "REJECTED") {
    const reasons = audio.quality_reasons.map((reason) => reasonText[reason] ?? "서버가 음원을 거부했어요.");
    return `음원 검증 실패. ${reasons.join(" ") || reasonText[audio.rejection_code ?? ""] || "다른 원본으로 다시 시도해 주세요."}`;
  }
  if (audio.status === "VERIFYING") return "서버가 음원을 검증 중이에요. 같은 사건의 상태를 다시 확인해 주세요.";
  return "음원 처리가 끝나지 않았어요. 같은 사건의 상태를 확인해 주세요.";
}

function errorText(error: unknown): string {
  if (error instanceof ContractApiError) {
    if (error.kind === "permission" || error.kind === "not-found" || error.kind === "authentication") return "현재 아기 접근 권한을 확인할 수 없어요. 다시 로그인하거나 권한을 확인해 주세요.";
    if (error.status === 503 || error.status === 500) return "서버에 일시 장애가 있어요. 같은 요청으로 다시 확인해 주세요.";
    if (error.status === 429) return `요청이 제한됐어요. ${error.retryAfterSeconds ?? "잠시"}초 뒤 다시 시도해 주세요.`;
    if ([413, 415, 422].includes(error.status)) return "서버가 원본 음원을 거부했어요. 같은 사건의 검증 상태를 확인해 주세요.";
  }
  return error instanceof Error && !/https?:\/\//.test(error.message) ? error.message : "연결이 끊겼어요. 같은 요청의 결과를 확인해 주세요.";
}

/** One tab-local logical intake. Mutation IDs survive retries while this instance lives. */
export class AudioIntake {
  private view: IntakeView = { stage: "idle", message: "녹음하거나 파일을 선택해 주세요.", progress: 0, episode: null, audio: null, grant: null, source: null };
  ...
  private assertCurrent(): string {
    ...
    if (!this.snapshot.userId || this.snapshot.babyId !== this.babyId) throw new Error("현재 아기 범위가 바뀌었어요.");
    return this.snapshot.userId;
  }

  select(source: Source, blob: Blob, duration: number | null, preferResumable: boolean): void {
    this.assertCurrent();
    if (this.view.episode) throw new Error("진행 중인 사건을 먼저 완료하거나 취소해 주세요.");
    if (!blob.type) throw new Error("원본 파일의 MIME 형식을 확인할 수 없어요. 다른 파일을 선택해 주세요.");
    if (blob.size === 0 || blob.size > MAX_AUDIO_BYTES) throw new Error("파일은 1~25,000,000바이트여야 해요.");
    const limit = source === "MANUAL" ? MANUAL_MAX_SECONDS : FILE_MAX_SECONDS;
    if (duration !== null && duration > limit) throw new Error(`${limit}초 이내 음원만 사용할 수 있어요.`);
    this.blob = blob;
    this.duration = duration;
    this.startedAt = null;
    this.preferResumable = preferResumable;
    this.set({ stage: "selected", source, progress: 0, message: `원본 ${blob.size.toLocaleString()}바이트 · 실제 MIME ${blob.type}. ${duration === null ? "길이는 서버가 확인합니다." : `${duration.toFixed(1)}초.`}` });
  }

  selectRecording(recording: FinishedRecording, preferResumable: boolean): void {
    this.select("MANUAL", recording.blob, recording.durationSeconds, preferResumable);
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      if (!episode) {
        ...
        episode = requireData(await this.client.POST("/episodes", {
          params: { header: idempotencyHeaders(this.ids.episode) },
          body: { client_request_id: this.ids.episode, baby_id: this.babyId, source,
      ...
      if (!grant) {
        this.lastStep = "grant";
        this.set({ stage: "granting", message: "업로드 승인을 요청하고 있어요." });
        const allocated = requireData(await this.client.POST("/episodes/{episode_id}/uploads", {
          params: { path: { episode_id: episode.episode_id }, header: idempotencyHeaders(this.ids.grant) },
          body: { client_request_id: this.ids.grant, mime_type: this.blob.type, bytes: this.blob.size,
            duration_seconds: this.duration, checksum_sha256: null,
      ...
      if (Date.now() >= Date.parse(grant.expires_at)) { this.set({ stage: "expired", message: "15분 업로드 승인이 만료됐어요. 같은 음원으로 재승인을 요청해 주세요." }); return; }
      this.set({ stage: "transferring", message: `${grant.method} 방식으로 전송 중이에요.` });
  ...
  async complete(): Promise<void> {
    ...
    this.set({ stage: "completing", message: "서버가 실제 원본을 검증하고 있어요." });
    try {
      requireData(await this.client.POST("/uploads/{upload_id}/complete", {
  ...
  async renew(): Promise<void> {
    ...
    if (!audio || !oldGrant || !this.blob || audio.status !== "ALLOCATED" || Date.now() < Date.parse(oldGrant.expires_at)) return;
    try {
      const renewed = requireData(await this.client.POST("/audio-assets/{audio_id}/uploads", {
        params: { path: { audio_id: audio.audio_id }, header: idempotencyHeaders(this.ids.renew) },
        body: { client_request_id: this.ids.renew, version: audio.version },
      }));
      this.assertCurrent();
      if (renewed.upload.object_key !== oldGrant.object_key || renewed.upload.bucket !== oldGrant.bucket || renewed.audio.audio_id !== audio.audio_id) throw new Error("재승인 객체 경로가 달라졌어요.");
      clearAudioResume({ userId: this.snapshot.userId!, babyId: this.babyId, audioId: audio.audio_id, uploadId: oldGrant.upload_id });
      this.set({ audio: renewed.audio, grant: renewed.upload, stage: "selected", progress: 0, message: "같은 음원 경로로 재승인받았어요. 전송을 다시 시작해 주세요." });
    } catch (error) { this.set({ stage: "expired", message: errorText(error) }); }
  }
  ...
  async cancel(): Promise<void> {
    ...
    try {
      requireData(await this.client.POST("/uploads/{upload_id}/cancel", {
        params: { path: { upload_id: grant.upload_id }, header: idempotencyHeaders(this.ids.cancel) },
        body: { client_request_id: this.ids.cancel },
      }));
      this.blob = null;
      clearAudioResume({ userId: this.snapshot.userId!, babyId: this.babyId, audioId: grant.audio_id, uploadId: grant.upload_id });
      this.set({ stage: "cancelled", message: "전송을 중단하고 서버 승인을 취소했어요. 다시 보내려면 새 음원으로 시작해 주세요." });
    } catch (error) { this.set({ stage: "uncertain", message: `전송은 중단됐지만 서버 취소 결과가 불명확해요. ${errorText(error)}` }); }
  }
  ...
  async play(element: HTMLAudioElement): Promise<void> {
    ...
    this.clearPlayback();
    const consents = requireData(await this.client.GET("/consents", { params: { query: { baby_id: this.babyId } } }));
    this.assertCurrent();
    const retention = consents.items.filter((item) => item.baby_id === this.babyId && item.scope === "AUDIO_RETENTION")
      .sort((left, right) => right.version - left.version)[0];
    if (retention?.status !== "GRANTED") {
      this.set({ message: "보관 동의가 없어 재생 링크를 발급하지 않았어요." });
      return;
    }
    const playback = requireData(await this.client.GET("/audio-assets/{audio_id}/playback", { params: { path: { audio_id: this.view.audio.audio_id } } }));
    this.assertCurrent();
    const remaining = Date.parse(playback.expires_at) - Date.now();
    if (playback.audio_id !== this.view.audio.audio_id || remaining <= 0 || remaining > 60_000) throw new Error("재생 링크의 범위를 확인할 수 없어요.");
    const signed = new URL(playback.playback_url);
    if (signed.origin !== new URL(this.config.supabaseUrl).origin ||
        !signed.pathname.startsWith("/storage/v1/object/sign/baby-audio/") || signed.username || signed.password) {
    ...
    if (playbackRemaining <= 0) throw new Error("재생 링크가 만료됐어요. 다시 요청해 주세요.");
    ...
    this.playbackTimer = setTimeout(() => this.clearPlayback(), playbackRemaining);
    await element.play();
  }
