...
export function DraftProvider({
  ...
  const identityKey =
    real.status === "signed-in" && real.userId
      ? `real:${real.userId}`
...
function DraftStore({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  const [careEventDirty, setCareEventDirtyState] = useState<
    ReadonlySet<string>
  >(new Set());

  const getLiveText = useCallback(
    (babyId: string) => live[babyId] ?? "",
    [live],
  );
  const hasLiveText = useCallback(
    (babyId: string) => (live[babyId]?.trim().length ?? 0) > 0,
    [live],
  );
  const getSavedDraft = useCallback(
    (babyId: string) => saved[babyId] ?? null,
    [saved],
  );
  const hasCareEventDirty = useCallback(
    (babyId: string) => careEventDirty.has(babyId),
    [careEventDirty],
  );
  ...
  const saveLiveAsDraft = useCallback(
    (babyId: string) => {
  ...
  useEffect(() => {
    ...
    return scope.subscribe(() => {
      ...
      if (
        previous.babyId &&
        (previous.babyId !== next.babyId || previous.userId !== next.userId)
      )
        clearBaby(previous.babyId);
      previous = next;
    });
  }, [scope, clearBaby]);
